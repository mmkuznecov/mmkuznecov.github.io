...
// eigSym2 on a known symmetric matrix [[2,1],[1,2]] -> eigenvalues 3,1; vecs (1,1)/√2,(−1,1)/√2
{
  ...
  ok(
    "eigSym2 eigenvector",
    approx(Math.abs(e.v1[0]), Math.SQRT1_2, 1e-9) &&
      approx(Math.abs(e.v1[1]), Math.SQRT1_2, 1e-9),
  );
  ok(
    "eigSym2 orthogonal",
    approx(e.v1[0] * e.v2[0] + e.v1[1] * e.v2[1], 0, 1e-9),
  );
}
...
// jacobiEigvals on equicorrelated matrix: eigenvalues are 1+(k-1)ρ and 1-ρ (×(k-1))
{
  ...
  ok(
    "jacobi remaining eigenvalues",
    eigs.slice(1).every((l) => approx(l, 1 - rho, 1e-6)),
  );
  ok(
    "jacobi trace preserved",
    approx(
      eigs.reduce((a, b) => a + b, 0),
      k,
      1e-6,
    ),
  );
...
// quantizedCapacity: equicorrelated -> ~1 at ρ→0, climbs substantially toward k at ρ→1
{
  const k = 8,
    D = 0.6,
    diag = new Array(k).fill(1);
  const C0 = core.quantizedCapacity(
    core.jacobiEigvals(core.buildEquicorr(k, 0.001)),
    diag,
    D,
  );
  const C1 = core.quantizedCapacity(
    core.jacobiEigvals(core.buildEquicorr(k, 0.98)),
    diag,
    D,
  );
  ...
  ok(
    "capacity monotone in rho",
    (() => {
      let prev = 0;
      for (let r = 0; r <= 0.95; r += 0.05) {
        const C = core.quantizedCapacity(
          core.jacobiEigvals(core.buildEquicorr(k, r)),
          diag,
          D,
        );
...
// AR(1) accrues redundancy more slowly than equicorrelated at the same ρ (post's claim)
{
  const k = 8,
    ...
    diag = new Array(k).fill(1),
    rho = 0.7;
  const Cequi = core.quantizedCapacity(
    core.jacobiEigvals(core.buildEquicorr(k, rho)),
    diag,
    D,
  );
  const Car1 = core.quantizedCapacity(
    core.jacobiEigvals(core.buildAR1(k, rho)),
    diag,
    D,
  );
...
const posts = existsSync(join(ROOT, "posts"))
  ? readdirSync(join(ROOT, "posts")).filter((s) => {
      const d = join(ROOT, "posts", s);
      try {
        return (
          statSync(d).isDirectory() && existsSync(join(d, "demo", "demo.js"))
        );
      } catch {
        return false;
      }
...
if (posts.length === 0) {
  fail++;
  console.error(
    "  FAIL no built demo/demo.js found — run `node build.mjs` first",
  );
}
for (const slug of posts) {
  try {
    execFileSync(process.execPath, [
      "--check",
      join(ROOT, "posts", slug, "demo", "demo.js"),
    ]);
    ok(`posts/${slug}/demo/demo.js parses`, true);
  } catch (e) {
    ok(`posts/${slug}/demo/demo.js parses`, false);
