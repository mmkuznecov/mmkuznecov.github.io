...
boot({
  "pca-cloud": (root) => {
    ...
    const stage = new Stage(stageHost, {
      height: 360,
      pad: { l: 36, r: 18, t: 18, b: 30 },
      layout: (s) => s.fitEqual(SPAN),
    });

    stage.onDraw((c, s) => {
      s.grid(2);
      s.axisX(0);
      s.segment(0, s.ydom[0], 0, s.ydom[1], theme.muted, 1); // y-axis
      ...
      c.globalAlpha = 0.9;
      c.beginPath();
      c.ellipse(0, 0, s1 * sclx, s2 * sclx, 0, 0, 2 * Math.PI);
      c.stroke();
      c.restore();
    ...
    pts.forEach((p) =>
      stage.addDraggable({
        ...
        y: () => p.y,
        move: (nx, ny) => {
          p.x = clamp(nx, -SPAN, SPAN);
          p.y = clamp(ny, -SPAN, SPAN);
        },
        r: 16,
      }),
    );

    toggle(controls, "show projection onto PC1", false, (v) => {
      showResid = v;
      stage.render();
    });
    button(controls, "reset cloud", () => {
      const f = startCloud();
      pts.forEach((p, i) => {
        p.x = f[i].x;
        p.y = f[i].y;
    ...
    button(controls, "make it round", () => {
      pts.forEach((p, i) => {
        const a = (i / pts.length) * 2 * Math.PI;
        p.x = 2.6 * Math.cos(a) + jit();
        p.y = 2.6 * Math.sin(a) + jit();
      });
      stage.render();
    });
    ...
    function startCloud() {
      ...
      const out = [];
      const dirs = [
        -1.6, -1.1, -0.7, -0.3, 0.1, 0.4, 0.8, 1.2, 1.7, 2.1, -2.0, 0.6,
      ];
      for (let i = 0; i < dirs.length; i++) {
        const t = dirs[i] * 2.2;
        out.push({
          x: t + (Math.random() - 0.5) * 1.0,
          y: 0.55 * t + (Math.random() - 0.5) * 1.6,
        });
      }
      return out;
    }
