...
boot({
  "completion-capacity": (root) => {
    ...
    const stage = new Stage(stageHost, {
      height: 320,
      pad: { l: 40, r: 150, t: 16, b: 32 },
      layout: (s) => {
        s.setDomain([0, 0.98], [0, k + 0.5]);
      },
    });
    ...
    stage.onDraw((c, s) => {
      ...
      c.textAlign = "left";
      c.fillText("C", s.pad.l - 28, s.pad.t + 10);
      c.restore();
    ...
    function drawHeatmap(c, s) {
      ...
      c.textAlign = "center";
      c.fillText("covariance Σ", x0 + size / 2, y0 + size + 14);
      c.restore();
    }

    stage.addDraggable({
      x: () => rho,
      y: () => Math.min(capAt(rho), k + 0.5),
      move: (nx) => {
        rho = clamp(nx, 0, 0.98);
        rhoUI.set(rho);
      },
    ...
    segmented(
      controls,
      [
        { label: "equicorrelated", value: "equi" },
        { label: "AR(1)", value: "ar1" },
      ],
      model,
      (v) => {
        model = v;
